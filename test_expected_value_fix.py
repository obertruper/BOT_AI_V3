...
pred_dict = {
    ...
    "confidence": 0.75,
    "signal_strength": 0.8,
    "primary_returns": {"15m": 0.001, "1h": 0.002, "4h": 0.003, "12h": 0.004},
    "risk_level": "MEDIUM",
    "risk_metrics": {"max_drawdown_1h": 0.015, "max_drawdown_4h": 0.025, "volatility": 0.02},
    "stop_loss_pct": 0.02,
...
try:
    ...
    async def test():
        result = await processor._convert_predictions_to_signal(
            symbol="BTCUSDT", predictions=pred_dict, current_price=50000.0
        )

        if result:
            print("✅ Сигнал создан успешно!")
