...
class TraderFormatter(BaseFormatter):
    ...
    def __init__(self, trader_id: str, session_id: Optional[str] = None, include_metrics: bool = True):
        format_string = (
...
class StrategyFormatter(BaseFormatter):
    ...
    def __init__(self, strategy_name: str, trader_id: Optional[str] = None):
        format_string = (
            '%(isotime)s [STRATEGY:%(strategy_name)s'
        )
        if trader_id:
            format_string += ':%(trader_id)s'
        format_string += (
            '] %(levelname)-8s %(name)s - %(message)s | %(signal_info)s'
        )
...
class MLFormatter(BaseFormatter):
    ...
    def __init__(self, model_name: str, model_version: Optional[str] = None):
        format_string = (
            '%(isotime)s [ML:%(model_name)s'
        )
        if model_version:
            format_string += ':%(model_version)s'
        format_string += (
            '] %(levelname)-8s %(name)s - %(message)s | %(ml_info)s'
        )
...
def create_trader_formatter(trader_id: str, session_id: Optional[str] = None) -> TraderFormatter:
...
def create_strategy_formatter(strategy_name: str, trader_id: Optional[str] = None) -> StrategyFormatter:
    """Создание форматтера для стратегии"""
    return StrategyFormatter(strategy_name, trader_id)


def create_ml_formatter(model_name: str, model_version: Optional[str] = None) -> MLFormatter:
...
def create_json_formatter(component_type: str, component_id: Optional[str] = None) -> JSONStructuredFormatter:
...
def add_trader_context(record: logging.LogRecord, trader_id: str, metrics: Optional[Dict[str, Any]] = None):
...
def add_exchange_context(record: logging.LogRecord, exchange: str, api_metrics: Optional[Dict[str, Any]] = None):
...
def add_signal_context(record: logging.LogRecord, signal_type: str, symbol: str, confidence: Optional[float] = None):
...
def add_ml_context(record: logging.LogRecord, prediction: Any, confidence: Optional[float] = None, model_score: Optional[float] = None):
