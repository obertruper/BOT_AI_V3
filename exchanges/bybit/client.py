...
from ..base.models import (
    ...
    Kline, AccountInfo, ExchangeInfo, create_order_from_dict, 
    create_position_from_dict
)
from ..base.order_types import OrderRequest, OrderResponse, OrderStatus, OrderSide, OrderType
from ..base.exceptions import (
    ConnectionError, AuthenticationError, APIError, RateLimitError,
    InsufficientBalanceError, OrderError, PositionError, MarketDataError,
    create_api_error_from_response, is_rate_limit_error, extract_retry_after
)
...
class BybitClient(BaseExchangeInterface):
    ...
    def __init__(self, 
        ...
        self.logger = logger_factory.get_logger("bybit_client", component="exchange_bybit")
    ...
    async def _make_request(self, 
        ...
        if auth:
            ...
            sign_params = json.dumps(params) if method == "POST" and params else query_string
        ...
        for attempt in range(self.retry_count + 1):
            try:
                ...
                if status_code != 200:
                    if status_code in [429, 500, 502, 503, 504] and attempt < self.retry_count:
                        continue
                ...
                if ret_code != 0:
                    ...
                    retry_codes = [10002, 10006, 10018, 20001, 20022, 20025, 20027, 20033]
                ...
                if attempt < self.retry_count and not isinstance(e, (APIError, AuthenticationError)):
                    continue
                raise
        ...
        try:
            ...
            for item in instruments_list:
                instrument = Instrument(
                    ...
                    max_order_qty=float(item.get("lotSizeFilter", {}).get("maxOrderQty", "1000000")),
    ...
    async def get_account_info(self) -> AccountInfo:
        ...
        try:
            ...
            response = await self._make_request("GET", "/v5/account/wallet-balance", params, auth=True)
    ...
    async def get_balance(self, currency: str, account_type: Optional[str] = None) -> Balance:
    ...
    async def place_order(self, order_request: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            self.logger.info(f"Placing order: {symbol} {order_request.side.value} {order_request.quantity} {order_request.order_type.value}")
            
            # Выполнение запроса
            response = await self._make_request("POST", "/v5/order/create", params, auth=True)
    ...
    async def cancel_order(self, symbol: str, order_id: str) -> OrderResponse:
        ...
        try:
            ...
            response = await self._make_request("POST", "/v5/order/cancel", params, auth=True)
            result = response.get("result", {})
            ...
            raise OrderError("bybit", "cancellation", order_id=order_id, symbol=symbol, reason=str(e))
    ...
    async def cancel_all_orders(self, symbol: Optional[str] = None) -> List[OrderResponse]:
        ...
        try:
            ...
            response = await self._make_request("POST", "/v5/order/cancel-all", params, auth=True)
            result = response.get("result", {})
            ...
            for order_data in cancelled_orders:
                responses.append(OrderResponse.success_response(
    ...
    async def modify_order(self, symbol: str, order_id: str, 
        ...
        try:
            ...
            if not quantity and not price:
                raise ValueError("Either quantity or price must be specified for order modification")
            ...
            raise OrderError("bybit", "modification", order_id=order_id, symbol=symbol, reason=str(e))
        ...
        try:
            ...
            for pos_data in positions_list:
                ...
                position = create_position_from_dict({
                    ...
                    "liqPrice": pos_data.get("liqPrice", "0") if pos_data.get("liqPrice", "0") != "0" else None,
                    "stopLoss": pos_data.get("stopLoss", "0") if pos_data.get("stopLoss", "0") != "0" else None,
                    "takeProfit": pos_data.get("takeProfit", "0") if pos_data.get("takeProfit", "0") != "0" else None,
    ...
    async def set_leverage(self, symbol: str, leverage: float) -> bool:
        ...
        try:
            ...
            response = await self._make_request("POST", "/v5/position/set-leverage", params, auth=True)
    ...
    async def set_position_mode(self, symbol: str, hedge_mode: bool) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Setting position mode for {symbol}: {'Hedge' if hedge_mode else 'One-Way'}")
            
            response = await self._make_request("POST", "/v5/position/switch-mode", params, auth=True)
    ...
    async def set_stop_loss(self, symbol: str, stop_price: float, 
        ...
        try:
            ...
            response = await self._make_request("POST", "/v5/position/trading-stop", params, auth=True)
    ...
    async def set_take_profit(self, symbol: str, take_price: float,
                            quantity: Optional[float] = None) -> OrderResponse:
        """
