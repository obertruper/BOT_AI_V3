...
from .base.exceptions import ExchangeError, AuthenticationError
...
class ExchangeFactory:
    ...
    def __init__(self):
        ...
        self.logger = logger_factory.get_logger("exchange_factory", component="exchanges")
    ...
    def _register_exchanges(self):
        ...
        self.logger.info(f"Exchange factory initialized with {len(self._exchange_classes)} exchanges")
    ...
    def create_from_config(self, config: ExchangeConfig, 
                          cache_key: Optional[str] = None,
                          force_new: bool = False) -> BaseExchangeInterface:
        """
    ...
    def clear_cache(self, exchange_type: Optional[Union[str, ExchangeType]] = None):
        ...
        if exchange_type is None:
            ...
            self.logger.info(f"Cleared {len(keys_to_remove)} {exchange_type} clients from cache")
    ...
    def _validate_credentials(self, api_key: str, api_secret: str, 
                            exchange_type: ExchangeType):
        ...
        if exchange_type == ExchangeType.BYBIT:
            if len(api_key) < 10 or len(api_secret) < 10:
                raise AuthenticationError(
                    "bybit",
                    "Invalid Bybit API credentials format"
                )
...
def create_bybit_client(api_key: str, api_secret: str, 
    ...
    return factory.create_client(
        ExchangeType.BYBIT, api_key, api_secret, 
        sandbox=sandbox, **kwargs
    )


def create_binance_client(api_key: str, api_secret: str,
                         sandbox: bool = False, **kwargs) -> BaseExchangeInterface:
    """Быстрое создание Binance клиента (будет реализовано)"""
    factory = get_exchange_factory()
    return factory.create_client(
        ExchangeType.BINANCE, api_key, api_secret,
        sandbox=sandbox, **kwargs
    )
