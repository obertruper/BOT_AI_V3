    ...
    try:
        ...
        if positions:
            for pos in positions:
                logger.info(
                    f"Position {symbol}: leverage={pos.get('leverage', 'N/A')}, side={pos.get('side', 'N/A')}"
                )
        ...
        try:
            ...
            if response.get("retCode") == 0:
                ...
                if instruments:
                    ...
                    logger.info(
                        f"  Max leverage: {inst.get('leverageFilter', {}).get('maxLeverage', 'N/A')}"
                    )
                    logger.info(
                        f"  Min leverage: {inst.get('leverageFilter', {}).get('minLeverage', 'N/A')}"
                    )
...
async def fix_leverage_mode(client: BybitClient, symbol: str, leverage: float = 5.0):
    ...
    try:
        ...
        try:
            params = {"category": "linear", "symbol": symbol, "mode": 0}  # 0 = One-way mode
