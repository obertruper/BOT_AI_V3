            ...
            if isinstance(module, nn.Linear):
                ...
                if "direction_head" in name:
                    if module.out_features == 12:  # Финальный слой direction head
                        ...
                        if module.bias is not None and bias_init in [
                            "balanced",
                            "neutral",
                        ]:
                            ...
                            with torch.no_grad():
                                ...
                                if bias_init == "neutral":
                                    ...
                                    bias.fill_(0.0)
                                    self.logger.info(
                                        "🔧 Используется нейтральная инициализация bias (все = 0.0)"
                                    )

                                elif bias_init == "balanced":
                                    # Используем веса классов для смещения
                                    class_weights = self.config.get("loss", {}).get(
                                    ...
                                    self.logger.info(
                                        f"🎯 Смещение bias на основе весов: {bias[0].tolist()}"
                                    )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.temperature is not None:
            ...
            if not hasattr(self, "_temp_logged"):
                ...
                self.logger.info(
                    f"🌡️ Temperature scaling активирован: T={self.temperature.item():.2f}"
                )
        ...
        if low_confidence_mask.any():
            ...
            if not hasattr(self, "_confidence_logged"):
                ...
                self.logger.info(
                    f"🎯 Порог уверенности {confidence_threshold}: {n_low_confidence} предсказаний изменены на FLAT"
                )
        ...
        outputs._confidence_scores = (
            confidence_scores  # (B, 4) - уверенность для каждого таймфрейма
        )
    ...
    def forward(
        ...
        for timeframe in ["15m", "1h", "4h", "12h"]:
            ...
            wrong_direction_mask = (
                (predicted_direction == 0) & (actual_direction == 1)
            ...
            false_positive_penalty = (
                (predicted_direction != 2) & (actual_direction == 2)
            ).float() * 0.5

            # Комбинированный loss для таймфрейма
            timeframe_loss = (
                ce_loss * movement_weight + direction_penalty + false_positive_penalty
            ).mean()
...
class DirectionalMultiTaskLoss(nn.Module):
    ...
    def update_class_weights(self, targets: torch.Tensor):
        ...
        self.running_class_counts = (
            self.class_weight_momentum * self.running_class_counts
        ...
        if self.total_samples > 100:  # Начинаем адаптацию после 100 примеров
            ...
            self.class_weights = (
                1 - weight_update_rate
            ) * self.class_weights + weight_update_rate * new_weights
    ...
    def focal_loss(
        self, logits: torch.Tensor, targets: torch.Tensor, dynamic_weights: torch.Tensor
    ) -> torch.Tensor:
        """
            ...
            if hasattr(outputs, "_direction_logits"):
                ...
                if entropy_weight > 0:
                    ...
                    if self.auto_adjust_on_collapse and mean_entropy < self.collapse_threshold:
                        ...
                        if flat_ratio > 0.85:
                            print(
                                f"🚨 ОБНАРУЖЕНО СХЛОПЫВАНИЕ (flat_ratio={flat_ratio:.2f})! Экстренная корректировка весов."
                            )
                ...
                for i in range(4):  # Для каждого таймфрейма
                    ...
                    if not hasattr(self, "_debug_logged") and i == 0:
                        ...
                        self.logger.info(
                            f"   Логиты: LONG={logits_sample[0]:.3f}, SHORT={logits_sample[1]:.3f}, FLAT={logits_sample[2]:.3f}"
                        )
                        self.logger.info(
                            f"   Вероятности: LONG={probs_sample[0]:.3f}, SHORT={probs_sample[1]:.3f}, FLAT={probs_sample[2]:.3f}"
                        )
                        self.logger.info(
