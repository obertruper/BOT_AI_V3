...
class SignalQualityAnalyzer:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.strategy_params = {
            FilterStrategy.CONSERVATIVE: filter_config.get("conservative", {
            ...
            FilterStrategy.MODERATE: filter_config.get("moderate", {
            ...
            FilterStrategy.AGGRESSIVE: filter_config.get("aggressive", {
        ...
        logger.info(f"SignalQualityAnalyzer инициализирован с стратегией: {self.active_strategy.value}")
    ...
    def analyze_signal_quality(
        ...
        passed = self._check_filtering_criteria(
            directions, direction_probs, future_returns, risk_metrics,
            quality_metrics, params, rejection_reasons
        )
        ...
        if passed:
            ...
            stats['avg_quality'] = ((stats['avg_quality'] * (stats['passed'] - 1)) + 
                                  quality_metrics.quality_score) / stats['passed']
        else:
        ...
        logger.info(f"{status} сигнал {signal_type} (стратегия: {self.active_strategy.value}, качество: {quality_metrics.quality_score:.3f})")
    ...
    def _calculate_risk_score(self, risk_metrics: np.ndarray) -> float:
        ...
        if len(risk_metrics) >= len(self.timeframe_weights):
            weighted_risk = np.sum(risk_metrics[:len(self.timeframe_weights)] * self.timeframe_weights)
        else:
    ...
    def _check_filtering_criteria(
        ...
        if max_agreement < params["min_timeframe_agreement"]:
            rejection_reasons.append(f"Недостаточная согласованность ТФ: {max_agreement} < {params['min_timeframe_agreement']}")
            passed = False
        ...
        if low_confidence_count > 2:  # Максимум 2 ТФ могут иметь низкую уверенность
            rejection_reasons.append(f"Слишком много ТФ с низкой уверенностью: {low_confidence_count} > 2")
            passed = False
        ...
        if main_tf_confidence < main_tf_threshold:
            ...
            if (self.active_strategy == FilterStrategy.MODERATE and 
                params.get("alternative_main_plus_one", False)):
                
                alt_threshold = params.get("alternative_confidence_threshold", 0.75)
                high_conf_count = np.sum(confidences >= alt_threshold)
                
                if high_conf_count < 2:
                    rejection_reasons.append(f"Основной ТФ: {main_tf_confidence:.3f} < {main_tf_threshold:.3f}, альтернатива не выполнена")
                    passed = False
            else:
                rejection_reasons.append(f"Основной ТФ: {main_tf_confidence:.3f} < {main_tf_threshold:.3f}")
                passed = False
        ...
        if max_return < min_return_threshold:
            rejection_reasons.append(f"Низкая ожидаемая доходность: {max_return:.4f} < {min_return_threshold:.4f}")
            passed = False
        
        # 4. Проверка общего качества
        if quality_metrics.quality_score < params["min_quality_score"]:
            rejection_reasons.append(f"Низкое качество: {quality_metrics.quality_score:.3f} < {params['min_quality_score']:.3f}")
            passed = False
        
        # 5. Проверка силы сигнала
        signal_strength = 0.6 * quality_metrics.quality_score + 0.4 * quality_metrics.confidence_score
        if signal_strength < params["min_signal_strength"]:
            rejection_reasons.append(f"Слабый сигнал: {signal_strength:.3f} < {params['min_signal_strength']:.3f}")
            passed = False
        
        # 6. Проверка уровня риска
        risk_level = "LOW" if quality_metrics.risk_score < 0.3 else "MEDIUM" if quality_metrics.risk_score < 0.7 else "HIGH"
    ...
    def analyze_signal(
        ...
        logger.debug(f"   Agreement: {agreement_score:.3f}, Confidence: {confidence_score:.3f}, "
    ...
    def _calculate_agreement_score(self, directions: np.ndarray) -> float:
        ...
        weights = np.array(list(self.timeframe_weights.values()))
    ...
    def _calculate_confidence_score(self, confidences: np.ndarray, direction_probs: List[np.ndarray]) -> float:
        """
    ...
    def _calculate_consistency_score(self, directions: np.ndarray, returns: np.ndarray) -> float:
        ...
        for i, (direction, ret) in enumerate(zip(directions, returns)):
    ...
    def _get_timeframe_agreement_details(self, directions: np.ndarray) -> Dict:
        ...
        details = {}
        for i, (tf, direction) in enumerate(zip(timeframes, directions)):
            details[tf] = direction_map[direction]
        
        # Добавляем статистику
        unique, counts = np.unique(directions, return_counts=True)
        for dir_idx, count in zip(unique, counts):
