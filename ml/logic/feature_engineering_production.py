...
class LoggerAdapter:
    ...
    def start_stage(self, stage_name, **kwargs):
        """Заглушка для совместимости"""
        self.logger.info(f"Starting stage: {stage_name}")

    def end_stage(self, stage_name, **kwargs):
        """Заглушка для совместимости"""
        self.logger.info(f"Completed stage: {stage_name}")

    def log_features_info(self, df, stage):
        """Заглушка для совместимости"""
        self.logger.info(f"Features info at {stage}: shape={df.shape}")


def get_logger(name):
    """Возвращает адаптер логгера"""
    return LoggerAdapter(name)


class ProductionFeatureEngineer:
    """Создание признаков для модели прогнозирования - PRODUCTION версия из обучающего файла"""

    def __init__(self, config: dict = None):
        self.config = config or {}
        self.logger = get_logger("ProductionFeatureEngineer")
        self.feature_config = config.get("features", {}) if config else {}
        self.scalers = {}
        ...
        # Database connection для BOT_AI_V3
        self.db_engine = None
        self._init_db_connection()

    def _init_db_connection(self):
        """Инициализация подключения к БД BOT_AI_V3"""
        try:
            # Используем порт 5555 для PostgreSQL в BOT_AI_V3
            db_url = os.getenv(
                "DATABASE_URL", "postgresql://obertruper:@localhost:5555/bot_trading_v3"
            )
            self.db_engine = create_engine(db_url, pool_size=10, pool_pre_ping=True)
        except Exception as e:
            self.logger.error(f"Ошибка подключения к БД: {e}")

    def get_btc_data_for_correlation(self, start_date, end_date):
        """Получение данных BTC для расчета корреляций из БД BOT_AI_V3"""
        try:
            query = f"""
            SELECT datetime, symbol, close, volume, high, low, open
            ...
            ORDER BY datetime
            """
            df_btc = pd.read_sql(query, self.db_engine)
            df_btc["datetime"] = pd.to_datetime(df_btc["datetime"])
            return df_btc
        except Exception as e:
            self.logger.error(f"Ошибка получения данных BTC: {e}")
            return pd.DataFrame()
    ...
    def create_features(
        ...
        if not self.disable_progress:
            self.logger.start_stage("feature_engineering", symbols=df["symbol"].nunique())
        ...
        # ИСПРАВЛЕНО: cross-asset features всегда создаются (с заглушками для одиночных символов)
        ...
        if not self.disable_progress:
            self.logger.end_stage("feature_engineering", total_features=len(result_df.columns))
    ...
    def _create_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Используем базовую конфигурацию если не задана
        tech_config = self.feature_config.get("technical", [])

        # SMA - создаем базовые индикаторы
        sma_config = next((c for c in tech_config if c.get("name") == "sma"), None)
        if sma_config and "periods" in sma_config:
            periods = sma_config["periods"]
        else:
            # Используем стандартные периоды
            periods = [5, 10, 20, 50]
        ...
        # EMA - создаем всегда
        ema_config = next((c for c in tech_config if c.get("name") == "ema"), None)
        ema_periods = (
            ema_config["periods"] if ema_config and "periods" in ema_config else [10, 20, 50]
        )
        ...
        # RSI - создаем всегда
        rsi_config = next((c for c in tech_config if c.get("name") == "rsi"), None)
        rsi_period = rsi_config["period"] if rsi_config and "period" in rsi_config else 14
        ...
        # MACD - создаем всегда с дефолтными параметрами
        macd_config = next((c for c in tech_config if c.get("name") == "macd"), None)
        macd_slow = macd_config["slow"] if macd_config and "slow" in macd_config else 26
        macd_fast = macd_config["fast"] if macd_config and "fast" in macd_config else 12
        macd_signal = macd_config["signal"] if macd_config and "signal" in macd_config else 9
        ...
        # Bollinger Bands
        # Bollinger Bands - создаем всегда
        bb_config = next((c for c in tech_config if c.get("name") == "bollinger_bands"), None)
        bb_period = bb_config["period"] if bb_config and "period" in bb_config else 20
        bb_std_dev = bb_config["std_dev"] if bb_config and "std_dev" in bb_config else 2
        ...
        # ATR - создаем всегда
        atr_config = next((c for c in tech_config if c.get("name") == "atr"), None)
        atr_period = atr_config["period"] if atr_config and "period" in atr_config else 14
        ...
        # 13. Aroon Indicator - исправлено (нужны high и low, не close)
    ...
    def _create_rally_detection_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["volatility_squeeze_duration"] = df["volatility_squeeze"].groupby(squeeze_group).cumsum()
    ...
    def _create_futures_specific_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["cascade_risk"] = (
            (df["volume_spike"] == 1) & (abs(df["returns"]) > df["returns"].rolling(96).std() * 2)
        ).astype(int)
    ...
    def _create_ml_optimized_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 8. Cross-sectional features - создаем всегда (с заглушками)
        if (
            "btc_returns" in df.columns
            and not df["btc_returns"].isna().all()
            and df["btc_returns"].var() > 1e-10
        ):
            # Beta к BTC (реальный расчет)
            df["btc_beta"] = (
            ...
            # Заглушки если нет BTC данных
            df["btc_beta"] = 0.5  # нейтральная бета
            df["idio_vol"] = returns.rolling(50).std()  # используем обычную волатильность
        ...
        # 11. Order flow imbalance persistence - создаем заглушку если OFI отсутствует
        if "order_flow_imbalance" in df.columns:
            ...
            # Заглушка - создаем простую аппроксимацию через autocorrelation объема
            df["ofi_persistence"] = (
                df["volume"]
                .pct_change()
                .rolling(20)
        ...
        # Добавляем все признаки (теперь создаются безусловно)
    ...
    def _normalize_walk_forward(self, df: pd.DataFrame, train_end_date: str) -> pd.DataFrame:
        ...
        # Целевые переменные
        target_cols = [
            col for col in df.columns if col.startswith(("target_", "future_", "optimal_"))
        ]
        exclude_cols.extend(target_cols)
    ...
    def create_features_with_train_split(
        ...
        # Целевые переменные
        target_cols = [
            col for col in train_data.columns if col.startswith(("target_", "future_", "optimal_"))
        ]
        exclude_cols.extend(target_cols)
        ...
        for symbol in norm_iterator:
            ...
            if len(train_symbol_data) > 0:
                ...
                if train_valid_mask.sum() > 0:
                    ...
                    train_data.loc[train_valid_mask, numeric_feature_cols] = self.scalers[
                        symbol
    ...
    def _add_enhanced_features(
        ...
        """
        try:
            from ml.logic.enhanced_features import EnhancedFeatureEngineer
        except ImportError:
            self.logger.warning(
