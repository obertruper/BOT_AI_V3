...
class PatchTSTAdapter(BaseModelAdapter):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Оптимизации
        self.use_torch_compile = os.environ.get("TORCH_COMPILE_DISABLE", "").lower() not in (
            "1",
            "true",
        )

        logger.info(
            f"PatchTSTAdapter initialized with context_length={self.context_length}, "
    ...
    async def predict(self, data: np.ndarray | pd.DataFrame, **kwargs) -> np.ndarray:
        """
    ...
    def interpret_outputs(
        ...
        **kwargs,
        ...
        for i, (tf, logits) in enumerate(zip(timeframes, direction_logits_reshaped, strict=False)):
        ...
        unified_prediction = UnifiedPrediction(
            ...
            # Качество
            quality_score=(
                filter_result.quality_metrics.quality_score if filter_result.passed else 0.0
            ),
            agreement_score=(
        ...
        logger.info(
            f"PatchTST interpretation complete: {signal_type} signal with confidence {confidence:.2f}"
        )
