...
class ModelAdapterFactory:
    ...
    @classmethod
    def create_adapter(cls, model_type: str, config: dict[str, Any]) -> BaseModelAdapter:
        """
    ...
    @classmethod
    def register_adapter(cls, model_type: str, adapter_class: type[BaseModelAdapter]) -> None:
        """
    ...
    @classmethod
    def create_from_config(cls, config: dict[str, Any]) -> BaseModelAdapter | None:
        """
        ...
        if not ml_config.get("enabled", False):
            logger.warning(
                f"ML is disabled in configuration: enabled={ml_config.get('enabled', 'NOT_FOUND')}"
            )
            return None
        ...
        logger.debug(
            f"🔍 Model {active_model_name} config: enabled={model_config.get('enabled', True)}"
        )

        # Проверяем, включена ли модель
        if not model_config.get("enabled", True):
            logger.warning(
                f"Model {active_model_name} is disabled: enabled={model_config.get('enabled', 'NOT_FOUND')}"
            )
            return None
    ...
    @classmethod
    def validate_config(cls, config: dict[str, Any], model_type: str) -> tuple[bool, list[str]]:
        """
