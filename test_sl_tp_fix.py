...
async def test_sl_tp_setup():
    ...
    print(f"Тип: MARKET (рыночный)")
    print(f"Сторона: BUY (длинная позиция)")
    ...
    try:
        ...
        if response.success:
            ...
            if position and position.is_open:
                ...
                print(f"   Stop Loss: ${position.stop_loss:.2f}" if position.stop_loss else "   Stop Loss: ❌ не установлен")
                print(f"   Take Profit: ${position.take_profit:.2f}" if position.take_profit else "   Take Profit: ❌ не установлен")
                ...
                print(f"Stop Loss: {'✅ ' + str(position.stop_loss) if position.stop_loss else '❌ не установлен'}")
                print(f"Take Profit: {'✅ ' + str(position.take_profit) if position.take_profit else '❌ не установлен'}")
