...
def generate_signature(api_secret: str, params_str: str) -> str:
    ...
    return hmac.new(
        api_secret.encode("utf-8"), params_str.encode("utf-8"), hashlib.sha256
    ).hexdigest()
...
async def test_direct_api_call():
    ...
    async with aiohttp.ClientSession() as session:
            ...
            if data.get("retCode") == 0:
                ...
                if positions:
                    for pos in positions:
                        logger.info(
                            f"Position mode: {pos.get('positionIdx')} (0=OneWay, 1=BuyHedge, 2=SellHedge)"
                        )
        ...
        async with session.get(url, params=params) as response:
            ...
            if data.get("retCode") == 0:
                ...
                if instruments:
                    inst = instruments[0]
                    logger.info(
                        f"Min order qty: {inst.get('lotSizeFilter', {}).get('minOrderQty')}"
                    )
                    logger.info(f"Qty step: {inst.get('lotSizeFilter', {}).get('qtyStep')}")
                    logger.info(
                        f"Min leverage: {inst.get('leverageFilter', {}).get('minLeverage')}"
                    )
                    logger.info(
                        f"Max leverage: {inst.get('leverageFilter', {}).get('maxLeverage')}"
                    )
                    logger.info(
                        f"Leverage step: {inst.get('leverageFilter', {}).get('leverageStep')}"
                    )
        ...
        logger.info(
            f"Headers: {json.dumps({k: v[:20] + '...' if k == 'X-BAPI-SIGN' else v for k, v in headers.items()}, indent=2)}"
        )
        ...
        async with session.post(url, json=order_params, headers=headers) as response:
            ...
            if data.get("retCode") == 0:
                ...
                if error_code == 10001:
                    ...
                    logger.info(
                        "1. Check if symbol format is correct (should be BTCUSDT, not BTCUSDT.P)"
                    )
