...
class DocumentationDownloader:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            ...
            headers={"User-Agent": "BOT-Trading-v3-Docs-Downloader/1.0"},
        )
        return self
...
async def main():
    ...
    parser.add_argument("--library", type=str, help="Обновить конкретную библиотеку")
    parser.add_argument(
        "--check-updates",
        action="store_true",
    ...
    async with DocumentationDownloader() as downloader:
        if args.status:
            ...
            for library, info in status.items():
                print(
                    f"{library:15} | Последнее обновление: {info['last_update']:16} | "
