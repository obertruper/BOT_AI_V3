...
class TestGenerator:
    ...
    def analyze_function(self, func_node: ast.FunctionDef, class_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        for node in ast.walk(func_node):
            ...
            if isinstance(node, ast.Name) and any(db in node.id.lower() for db in ['pool', 'session', 'repository', 'db']):
                analysis['has_db_access'] = True
            
            # Проверка на внешние API
            if isinstance(node, ast.Attribute) and any(api in str(node.attr).lower() for api in ['request', 'post', 'get', 'websocket']):
    ...
    def _generate_test_scenarios(self, analysis: Dict) -> List[Dict]:
        ...
        if analysis['has_db_access']:
            scenarios.append({
                'name': 'db_mock',
                'description': 'Тест с мокированием БД',
                'type': 'mock'
            })
...
'''
    ...
    def _generate_negative_test(self, analysis: Dict, is_async: bool) -> str:
        ...
        if analysis['parameters']:
            ...
            if analysis['class']:
                test_body += f"{indent}instance = {analysis['class']}()\n"
                test_body += f"{indent}with pytest.raises((TypeError, ValueError, AttributeError)):\n"
                if is_async:
    ...
    def _generate_exception_test(self, analysis: Dict, is_async: bool) -> str:
        ...
        if analysis['has_db_access']:
            test_body += f"{indent}with patch('asyncpg.create_pool', side_effect=Exception('DB Error')):\n"
    ...
    def _generate_performance_test(self, analysis: Dict, is_async: bool) -> str:
        ...
        test_body += f"{indent}assert elapsed < 1.0, f'Функция слишком медленная: {{elapsed:.2f}}s'\n"
    ...
    def _generate_mock_test(self, analysis: Dict, is_async: bool, mock_type: str) -> str:
        ...
        if 'db' in mock_type:
            ...
            test_body += f"{indent}mock_api.get.return_value.json.return_value = {{'status': 'ok'}}\n"
        ...
        for category, modules in self.BUSINESS_MODULES.items():
            ...
            for module_path in modules:
                ...
                if tests:
                    ...
                    print(f"  ✅ {module_path}: {len(tests)} тестов → {test_file.relative_to(self.project_root)}")
    ...
    def generate_coverage_report(self) -> None:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            for category in self.BUSINESS_MODULES:
                ...
                if category_tests:
                    avg_complexity = sum(t['analysis']['complexity'] for t in category_tests) / len(category_tests)
                    f.write(f"| {category} | {len(set(t['module'] for t in category_tests))} | ")
                    f.write(f"{len(category_tests)} | {len(category_tests) * 5} | {avg_complexity:.1f} |\n")
