...
class MigrationVerifier:
    ...
    async def check_database_schema(self) -> Dict:
        ...
        async with get_async_db() as db:
            ...
            required_tables = [
                'orders', 'positions', 'trades', 'signals',
                'raw_market_data', 'processed_market_data',
            ...
            if table_checks.get('positions'):
                trailing_fields = ['trailing_stop_active', 'trailing_stop_price', 'trailing_stop_callback']
    ...
    async def check_enhanced_sltp(self) -> Dict:
        ...
        try:
            ...
            checks["profit_protection_configured"] = (
                profit_protection.get("enabled", False) and
                profit_protection.get("breakeven") is not None
            )
            
            # Trailing stop
            trailing = sltp_config.get("trailing_stop", {})
            checks["trailing_stop_configured"] = (
                trailing.get("enabled", False) and
                trailing.get("activation_percent") is not None
            )
    ...
    async def check_risk_management(self) -> Dict:
        ...
        try:
            ...
            checks["position_limits_set"] = (
                risk_config.get('max_positions') is not None and
                risk_config.get('max_position_size') is not None
            )
            
            # Проверка daily loss limit
            checks["daily_loss_limit_set"] = (
                risk_config.get('daily_loss_limit') is not None
            )
            
            # Проверка correlation management
            correlation = risk_config.get('correlation_management', {})
            checks["correlation_management"] = (
                correlation.get('enabled', False) and
                correlation.get('max_correlation') is not None
            )
    ...
    async def check_configuration(self) -> Dict:
        ...
        try:
            ...
            checks["ml_config_valid"] = (
                ...
                ml_config.get('model') is not None
            )
    ...
    async def check_performance(self) -> Dict:
        ...
        # Оценка
        performance_ok = (
            metrics["memory_usage_mb"] < 2048 and  # <2GB
            metrics["signal_generation_ms"] < 50 and  # <50ms
            metrics["db_query_ms"] < 10  # <10ms
        )
    ...
    async def check_trading_engine(self) -> Dict:
        ...
        try:
            ...
            required_methods = [
                'process_signals', 'create_order', 
                'execute_order', 'sync_positions'
            ]
