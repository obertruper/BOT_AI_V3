...
class StrategyFactory:
    ...
    def create_multiple(self, strategies_config: list[dict[str, Any]]) -> list[StrategyABC]:
        """
        ...
        for config in strategies_config:
            ...
            try:
                strategy = self.create_strategy(
                    name=config["name"], config=config.get("config", {})
                )
        ...
        try:
            with open(path, encoding="utf-8") as f:
                if path.suffix in [".yaml", ".yml"]:
                    ...
                    raise StrategyFactoryError(f"Unsupported config file format: {path.suffix}")
        except Exception as e:
    ...
    def export_config(self, strategy: StrategyABC, file_path: str, format: str = "yaml") -> None:
        """
