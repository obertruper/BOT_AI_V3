...
class StrategyManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Менеджер стратегий инициализирован. Загружено {len(self.strategies)} стратегий")
            return True
            ...
            for strategy_id, instance in self.strategies.items():
                ...
                if instance.state == StrategyState.ACTIVE and instance.task:
                    if instance.task.done():
                        ...
                        if exception:
                            self.logger.error(f"Стратегия {strategy_id} завершилась с ошибкой: {exception}")
                            instance.state = StrategyState.ERROR
                            instance.error_message = str(exception)
                
                # Автоматический перезапуск упавших стратегий
                if (instance.state == StrategyState.ERROR and 
                    self.restart_failed_strategies and
                    self._restart_attempts.get(strategy_id, 0) < self.max_restart_attempts):
    ...
    async def _update_strategy_metrics(self, instance: StrategyInstance):
        ...
        try:
            ...
            if hasattr(instance.strategy, 'get_metrics'):
                ...
                if strategy_metrics:
                    instance.metrics.signals_generated = strategy_metrics.get('signals_generated', 0)
                    instance.metrics.successful_trades = strategy_metrics.get('successful_trades', 0)
    ...
    def get_strategy_status(self, strategy_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'metrics': {
                ...
                'last_signal_time': instance.metrics.last_signal_time.isoformat() if instance.metrics.last_signal_time else None,
            },
    ...
    def get_summary_metrics(self) -> Dict[str, Any]:
        ...
        error_strategies = len([s for s in self.strategies.values() if s.state == StrategyState.ERROR])
        
        total_signals = sum(s.metrics.signals_generated for s in self.strategies.values())
        total_trades = sum(s.metrics.successful_trades + s.metrics.failed_trades for s in self.strategies.values())
        ...
        return {
            ...
            'paused_strategies': len([s for s in self.strategies.values() if s.state == StrategyState.PAUSED]),
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            if critical_strategies:
                self.logger.warning(f"Обнаружены неработающие критические стратегии: {[s.strategy_id for s in critical_strategies]}")
                return False
