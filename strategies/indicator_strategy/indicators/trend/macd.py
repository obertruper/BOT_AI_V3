...
class MACDIndicator(IndicatorBase):
    ...
    def calculate(self, data: pd.DataFrame) -> IndicatorResult:
        ...
        # Определение сигнала
        signal = self._determine_signal(macd_line, signal_line, histogram)
    ...
    def _check_divergence(self, prices: pd.Series, macd: pd.Series) -> Optional[str]:
        ...
        if (
            len(price_trough_idx) >= 2
            and len(macd_trough_idx) >= 2
        ...
        if (
            len(price_peak_idx) >= 2
            and len(macd_peak_idx) >= 2
