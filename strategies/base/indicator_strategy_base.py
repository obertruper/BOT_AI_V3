...
from .strategy_abc import StrategyABC, MarketData, TradingSignal, RiskParameters, SignalType
...
class IndicatorStrategyBase(StrategyABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Веса для скоринга
        self.base_weights = self.scoring_config.get('base_weights', {
            'trend': 0.30,
            'momentum': 0.25,
            'volume': 0.25,
            'volatility': 0.20
        ...
        self.regime_multipliers = self.scoring_config.get('regime_multipliers', {
    ...
    def _update_market_history(self, market_data: MarketData) -> None:
        ...
        if len(self.market_data_history[symbol]) > self.max_history_length:
            self.market_data_history[symbol] = \
                self.market_data_history[symbol].iloc[-self.max_history_length:]
    ...
    async def _detect_market_regime(self, symbol: str) -> MarketRegime:
        ...
        Returns:
            Режим рынка
        """
        if symbol not in self.market_data_history or \
           len(self.market_data_history[symbol]) < 50:
    ...
    def _generate_signal_from_score(self,
