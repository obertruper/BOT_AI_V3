...
class ResponseCollector:
    ...
    async def collect_ai_response(
        self, ai_system: str, chat_id: str, expected_length: str = "medium"
    ) -> AIResponse:
        """
        ...
        try:
            ...
            logger.info(
                f"Ответ от {ai_system} собран: {response.word_count} слов, "
    ...
    async def _wait_for_response_start(self, ai_system: str):
        ...
        try:
            await mcp__playwright__browser_wait_for(text="typing", time=10)
        except:
    ...
    async def _finalize_response(self, response: AIResponse):
        ...
        logger.info(
            f"Ответ от {response.ai_system} финализирован: "
...
class EnhancedResponseHandler:
    ...
    async def enhanced_send_task_to_all_chats(
        ...
        """
        logger.info(
            f"Улучшенная отправка задачи {task.task_id} с ожиданием {expected_length} ответов"
        )
        ...
        ai_responses = await self.collector.collect_multiple_responses(
            list(chat_ids.keys()), chat_ids, expected_length
        )
        ...
        for ai_system, ai_response in ai_responses.items():
            ...
            if ai_system in task.chat_sessions:
                ...
                session.metadata = {
                    ...
                    "collection_time": (
                        ai_response.end_time - ai_response.start_time
                    ).total_seconds(),
                }
