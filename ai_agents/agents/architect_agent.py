...
class ArchitectAgent:
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        avg_complexity = sum(m.complexity for m in self.modules.values()) / total_modules if total_modules > 0 else 0
        avg_doc_coverage = sum(m.docstring_coverage for m in self.modules.values()) / total_modules if total_modules > 0 else 0
        ...
        return {
            ...
            'dependency_graph_density': nx.density(self.dependency_graph) if self.dependency_graph else 0
        }
...
async def analyze_project_architecture(project_root: Optional[str] = None) -> ArchitectureAnalysis:
...
async def generate_architecture_report(output_file: str = "architecture_report.md") -> str:
