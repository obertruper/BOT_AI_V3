...
class AutonomousDeveloper:
    ...
    async def develop_feature(self, task: DevelopmentTask) -> DevelopmentResult:
        ...
        try:
            ...
            self._log(f"Error in {self.current_phase.value} phase", {"error": str(e)}, level="error")
...
ВАЖНО: Реализуйте полностью, не оставляйте заглушки."""
    ...
    async def _test_phase(self, plan: DevelopmentPlan, implementation: Dict[str, Any]) -> Dict[str, Any]:
...
Используйте fixtures и параметризацию где возможно."""
    ...
    async def _refine_phase(self, plan: DevelopmentPlan, test_results: Dict[str, Any]) -> Dict[str, Any]:
...
НЕ меняйте тесты чтобы они проходили - исправляйте код."""
    ...
    def _log(self, message: str, data: Optional[Dict[str, Any]] = None, level: str = "info"):
