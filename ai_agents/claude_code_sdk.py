...
class ClaudeCodeOptions:
    ...
    max_turns: int = 5
    allowed_tools: List[str] = field(default_factory=lambda: ["Read", "Write", "Edit", "Bash", "Task"])
...
class ClaudeCodeSDK:
    ...
    def __init__(self, use_package_auth: bool = True):
        ...
        if not use_package_auth:
            ...
            if not self.api_key:
                raise ValueError("API key required when use_package_auth=False. Set ANTHROPIC_API_KEY environment variable")
        else:
    ...
    def _verify_claude_auth(self):
        ...
        try:
            ...
            raise ValueError("Claude CLI not available or not authenticated. Please run 'claude' to authenticate.")
        except json.JSONDecodeError:
            raise ValueError("Invalid Claude CLI configuration.")
        
    async def query(self, prompt: str, options: Optional[ClaudeCodeOptions] = None, agent_name: str = "unknown") -> str:
    ...
    async def stream(self, prompt: str, options: Optional[ClaudeCodeOptions] = None) -> AsyncIterator[str]:
    ...
    async def batch(self, tasks: List[Dict[str, Any]], parallel: bool = True) -> List[str]:
        ...
        if parallel:
            results = await asyncio.gather(*[
                self.query(task["prompt"], task.get("options"))
                for task in tasks
            ])
        else:
            results = []
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # Сессия с контекстом
        session = sdk.create_session("dev_session", ClaudeCodeOptions(
            thinking_mode=ThinkingMode.THINK,
            max_turns=10
        ))
