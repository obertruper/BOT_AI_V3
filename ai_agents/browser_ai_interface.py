...
class BrowserAIInterface:
    ...
    async def query_model_via_browser(
        ...
        playwright_task = self._build_playwright_task(
            model_config, prompt, file_paths, use_existing_tab
        )
    ...
    def _build_playwright_task(
        ...
        if use_existing_tab:
            task_parts.extend(
                [
                    ...
                    "   - Если есть - выберите её с browser_tab_select",
        ...
        task_parts.extend(
            [
                ...
                "   - Нажмите её: browser_click",
                "",
                f"{'5' if file_paths else '4'}. ОЖИДАНИЕ ОТВЕТА:",
                "   - Дождитесь исчезновения индикаторов загрузки:",
            ]
        )
        ...
        task_parts.extend(
            [
                ...
                "   - Сделайте снимок экрана для проверки",
                f"   - Извлеките текст ответа из {model_config.selectors['response']}",
                "   - Верните ТОЛЬКО содержимое ответа AI (без интерфейсных элементов)",
                "",
                "ВАЖНЫЕ ПРАВИЛА:",
    ...
    async def compare_models(
        ...
        for model, task in tasks:
            try:
                ...
                results[model] = {"status": "error", "response": None, "error": str(e)}
...
class TerminalAICommands:
    ...
    async def intelligent_ask(
        ...
        print("🧠 Анализирую сложность запроса...")
    ...
    async def research_topic(self, topic: str, verification: bool = True):
        ...
        if verification:
            ...
            response = await self.browser_interface.query_model_via_browser(
                "claude_web", research_prompt
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print("\n2️⃣ Сложный вопрос:")
        await intelligent_ask(
            "Проанализируйте архитектуру торговых ботов и оптимизацию производительности"
        )

        # 3. Принудительная кросс-проверка
        print("\n3️⃣ Принудительная кросс-проверка:")
        await intelligent_ask(
            "!кросс! Какие лучшие практики для машинного обучения?",
