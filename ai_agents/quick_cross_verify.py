...
async def quick_verify(description: str, task_content: str):
    ...
    print(f"🎯 AI системы: ChatGPT o3-pro, Grok v4, Claude Opus 4")
    print("=" * 60)
    ...
    try:
        ...
        for ai_system, session_info in status['chat_sessions'].items():
            ...
            print(f"   {status_emoji} {ai_system.upper()}: {session_info['responses_count']} ответов")
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print("\nПримеры:")
        print("   python ai_agents/quick_cross_verify.py \"Стратегия скальпинга\" \"Разработай стратегию скальпинга для BTC\"")
        print("   python ai_agents/quick_cross_verify.py \"Риск-менеджмент\" \"Как настроить stop-loss для крипто трейдинга\"")
        print("   python ai_agents/quick_cross_verify.py \"Архитектура бота\" \"Оптимальная архитектура для HFT бота\"")
        sys.exit(1)
