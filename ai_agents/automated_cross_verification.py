...
class AutomatedCrossVerification:
    ...
    async def _initialize_single_chat(self, task: CrossVerificationTask, ai_system: str):
        ...
        try:
            ...
            await mcp__playwright__browser_tab_new(url=ai_config["url"])
            
            # Получаем список вкладок для определения индекса
            tabs = await mcp__playwright__browser_tab_list()
            ...
            await mcp__playwright__browser_take_screenshot(filename=screenshot_path)
    ...
    async def send_task_to_all_chats(self, task_id: str, expected_length: str = "long") -> Dict[str, str]:
        """
    ...
    async def _send_prompt_to_single_chat(self, task, ai_system: str):
        ...
        await mcp__playwright__browser_tab_select(index=session.tab_index)
    ...
    async def _send_to_single_chat(self, task: CrossVerificationTask, ai_system: str) -> str:
        ...
        response_snapshot = await mcp__playwright__browser_snapshot()
...
async def main():
    ...
    if len(sys.argv) < 3:
        ...
        print("Пример: python automated_cross_verification.py 'Стратегия скальпинга' 'Разработай стратегию скальпинга для BTC'")
        sys.exit(1)
