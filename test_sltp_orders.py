...
async def test_market_order_with_sltp():
    ...
    client = BybitClient(api_key=api_key, api_secret=api_secret, sandbox=False)
    ...
    try:
        ...
        response = await client._make_request(
            "GET", "/v5/account/wallet-balance", balance_params, auth=True
        )
        ...
        logger.info(
            f"Размер позиции: {rounded_qty} BTC (${float(rounded_qty) * current_price:.2f})"
        )
        ...
        if order_response.success:
            logger.info("✅ Ордер создан успешно!")
            ...
            if positions:
                for position in positions:
                    ...
                    if position.stop_loss:
                        ...
                        logger.warning("   ⚠️ Stop Loss НЕ установлен!")

                    if position.take_profit:
                        logger.info(f"   ✅ Take Profit установлен: ${position.take_profit:.2f}")
                    else:
                        logger.warning("   ⚠️ Take Profit НЕ установлен!")
                    ...
                    if close_response.success:
                        logger.info("✅ Позиция закрыта успешно")
                    else:
