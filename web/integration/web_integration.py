...
from .dependencies import initialize_dependencies, cleanup_dependencies, dependency_context
...
class WebIntegration:
    ...
    async def _load_web_config(self):
        ...
        try:
            ...
            logger.warning(f"Не удалось загрузить веб-конфигурацию, используем значения по умолчанию: {e}")
    
    async def start_web_server(self, host: Optional[str] = None, port: Optional[int] = None):
        """
        Запуск веб-сервера
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "orchestrator": {
                ...
                "trader_manager": self.orchestrator.trader_manager is not None if self.orchestrator else False,
                "exchange_factory": self.orchestrator.exchange_factory is not None if self.orchestrator else False,
                "config_manager": self.orchestrator.config_manager is not None if self.orchestrator else False
            },
            "event_bridge": {
                "initialized": self.event_bridge is not None,
...
@asynccontextmanager
async def web_integration_context(orchestrator: SystemOrchestrator, 
                                host: str = "0.0.0.0", 
                                port: int = 8080):
    """
    ...
    try:
        ...
        server_task = asyncio.create_task(
            web_integration.start_web_server(host, port)
        )
...
def check_web_integration_requirements(orchestrator: SystemOrchestrator) -> Dict[str, bool]:
    """
    ...
    if orchestrator:
        requirements.update({
            "trader_manager_available": hasattr(orchestrator, 'trader_manager') and orchestrator.trader_manager is not None,
            "exchange_factory_available": hasattr(orchestrator, 'exchange_factory') and orchestrator.exchange_factory is not None,
            "config_manager_available": hasattr(orchestrator, 'config_manager') and orchestrator.config_manager is not None,
            "health_checker_available": hasattr(orchestrator, 'health_checker') and orchestrator.health_checker is not None
        })
