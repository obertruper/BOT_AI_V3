...
from .mock_services import MockUserManager, MockSessionManager, MockStatsService
...
class WebOrchestratorBridge:
    ...
    def __init__(self, orchestrator=None):
        ...
        self.logger = logger_factory.get_logger("web_bridge", component="web_integration")
        
        # Core компоненты
        self.orchestrator = orchestrator
        self.trader_manager = getattr(orchestrator, 'trader_manager', None) if orchestrator else None
        self.exchange_factory = getattr(orchestrator, 'exchange_factory', None) if orchestrator else None
        self.config_manager = getattr(orchestrator, 'config_manager', None) if orchestrator else None
    ...
    async def get_traders(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self.trader_manager:
                ...
                return [self.data_adapter.trader_to_response(trader) for trader in traders]
        ...
        try:
            if self.use_mock:
                await self.event_bridge.emit_event(
                    ...
                    {"trader_id": trader_id, "status": "active", "message": "Трейдер запущен"}
                )
                return True
    ...
    async def stop_trader(self, trader_id: str) -> bool:
        ...
        try:
            if self.use_mock:
                await self.event_bridge.emit_event(
                    EventType.TRADER_STATUS_CHANGED,
                    {"trader_id": trader_id, "status": "stopped", "message": "Трейдер остановлен"}
                )
                return True
