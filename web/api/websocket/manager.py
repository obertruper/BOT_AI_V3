...
class WebSocketManager:
    ...
    async def disconnect(self, connection_id: str, code: int = 1000, reason: str = ""):
        ...
        logger.info(f"WebSocket соединение отключено: {connection_id} (код: {code}, причина: {reason})")
        ...
        for connection_id in subscribers:
            if connection_id in self.connections:
                try:
                    ...
                    logger.warning(f"Ошибка отправки сообщения в поток {stream} для соединения {connection_id}: {e}")
        ...
        for connection_id in connection_ids:
            if connection_id in self.connections:
                try:
                    ...
                    logger.warning(f"Ошибка отправки сообщения пользователю {user_id} для соединения {connection_id}: {e}")
                    # Удаляем неработающее соединение
                    await self.disconnect(connection_id, code=1011, reason="Send error")
        ...
        for connection_id in connection_ids:
            if connection_id in self.connections:
                try:
                    ...
                    logger.warning(f"Ошибка broadcast сообщения для соединения {connection_id}: {e}")
                    # Удаляем неработающее соединение
                    await self.disconnect(connection_id, code=1011, reason="Send error")
