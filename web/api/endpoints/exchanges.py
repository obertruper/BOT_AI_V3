...
async def get_exchange_info(exchange_name: str):
    ...
    try:
        ...
        if not exchange_client:
            raise HTTPException(status_code=404, detail=f"Биржа {exchange_name} не найдена или не подключена")
...
async def connect_exchange(config: ExchangeConfig):
    ...
    try:
        ...
        if not exchange_factory.is_supported(config.exchange):
            raise HTTPException(
                status_code=400, 
                detail=f"Биржа {config.exchange} не поддерживается"
            )
        ...
        exchange_client = await exchange_factory.create_exchange(config.exchange, exchange_config)
        ...
        logger.info(f"Биржа {config.exchange} успешно подключена", extra={
            'sandbox': config.sandbox
        })
