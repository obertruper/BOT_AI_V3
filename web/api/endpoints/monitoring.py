...
logger = logger_factory.get_logger("monitoring_api", component="web_api")
...
async def get_system_metrics():
    ...
    try:
        ...
        logger.debug("Получены системные метрики", extra={
            'cpu': cpu_percent,
...
async def get_trading_statistics(
    ...
    try:
        ...
        logger.info(f"Получена статистика торговли за {period}", extra={
...
async def get_alerts(
    ...
    try:
        ...
        logger.info(f"Получено {len(alerts)} алертов", extra={
            'filters': filters,
            'count': len(alerts)
        })
...
async def get_exchanges_status():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения статуса бирж: {str(e)}")
    ...
    try:
        ...
        overview = {
            ...
            "system": {
                ...
                "uptime_hours": system_metrics.uptime_seconds / 3600 if hasattr(system_metrics, 'uptime_seconds') else 0
            },
...
async def get_system_logs(
    ...
    try:
        ...
        logger.info(f"Получено {len(logs)} записей логов", extra={
            'filters': filters,
            'count': len(logs)
        })
