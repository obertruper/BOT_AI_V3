...
async def login(request: LoginRequest):
    ...
    try:
        ...
        logger.info(f"Успешный вход пользователя {request.username}", extra={
            'user_id': user.user_id,
...
async def logout(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error(f"Ошибка при выходе пользователя {current_user.get('username', 'unknown')}: {e}")
...
async def get_current_user_info(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error(f"Ошибка получения информации о пользователе {current_user.get('user_id')}: {e}")
...
async def change_password(
    ...
    try:
        ...
        logger.error(f"Ошибка изменения пароля для пользователя {current_user.get('username')}: {e}")
        raise HTTPException(status_code=500, detail=f"Ошибка изменения пароля: {str(e)}")
