...
from core.traders.trader_state import TraderState
...
async def get_traders(
    ...
    try:
        ...
        logger.info(f"Возвращено {len(filtered_traders)} трейдеров", extra={
            'total_count': len(traders),
            'filtered_count': len(filtered_traders),
            'filters': {'active_only': active_only, 'exchange': exchange, 'strategy': strategy}
        })
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения трейдеров: {str(e)}")
...
async def get_trader(trader_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения трейдера: {str(e)}")
...
async def create_trader(request: TraderCreateRequest):
    ...
    try:
        ...
        logger.info(f"Создан новый трейдер {request.trader_id}", extra={
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка создания трейдера: {str(e)}")
...
async def update_trader(trader_id: str, request: TraderUpdateRequest):
    ...
    try:
        ...
        logger.info(f"Обновлены настройки трейдера {trader_id}", extra={
            'updates': updates
        })
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка обновления трейдера: {str(e)}")
...
async def trader_action(trader_id: str, request: TradingActionRequest):
    ...
    try:
        ...
        if request.action == "start":
            ...
            result = {"action": "start", "status": "success", "message": "Торговля запущена"}
            
        elif request.action == "stop":
            await trader.stop_trading(force=request.force)
            result = {"action": "stop", "status": "success", "message": "Торговля остановлена"}
            
        elif request.action == "pause":
            await trader.pause_trading()
            result = {"action": "pause", "status": "success", "message": "Торговля приостановлена"}
            
        elif request.action == "resume":
            await trader.resume_trading()
            result = {"action": "resume", "status": "success", "message": "Торговля возобновлена"}
            
        else:
            raise HTTPException(status_code=400, detail=f"Неизвестное действие: {request.action}")
        
        logger.info(f"Выполнено действие {request.action} для трейдера {trader_id}", extra={
...
async def delete_trader(trader_id: str, force: bool = Query(False, description="Принудительное удаление")):
    """Удалить трейдера"""
    try:
        trader_manager = get_trader_manager()
        trader = trader_manager.get_trader(trader_id)
        ...
        logger.info(f"Трейдер {trader_id} удален", extra={
            'force': force
        })
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка удаления трейдера: {str(e)}")

@router.get("/{trader_id}/performance", response_model=Dict[str, Any])
async def get_trader_performance(trader_id: str, period: str = Query("24h", description="Период анализа")):
