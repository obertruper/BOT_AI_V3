...
async def get_prediction_visualization(
    ...
    try:
        ...
        if chart_type == "interactive":
            ...
            raise HTTPException(
                status_code=400, detail=f"Неподдерживаемый тип графика: {chart_type}"
            )

    except Exception as e:
        logger.error(f"Ошибка создания визуализации для {symbol}: {e}")
        raise HTTPException(status_code=500, detail=f"Ошибка создания визуализации: {e!s}")


@router.get("/features/{symbol}", response_model=list[FeatureImportance])
async def get_feature_importance(
    symbol: str, limit: int = Query(50, description="Количество топ признаков")
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка: {e!s}")
...
async def generate_ml_report(
    ...
    try:
        ...
        background_tasks.add_task(
            generate_comprehensive_report, report_id, symbols, include_features
        )
...
async def download_report(report_id: str):
    ...
    try:
        ...
        return FileResponse(
            path=str(report_file), media_type="application/json", filename=f"{report_id}.json"
        )
...
def create_static_chart(symbol: str, prediction: dict, market_data: pd.DataFrame) -> str:
    ...
    axes[1, 1].bar([signal_type], [confidence], color=signal_colors.get(signal_type, "gray"))
    ...
    buffer = io.BytesIO()
    plt.savefig(
        buffer, format="png", dpi=150, bbox_inches="tight", facecolor="black", edgecolor="black"
    )
    plt.close()
            ...
            try:
                ...
                if market_data is not None:
                    ...
                    report_data["charts"].append(
                        {"symbol": symbol, "status": "no_data", "error": "Нет данных"}
                    )
            except Exception as e:
                report_data["charts"].append({"symbol": symbol, "status": "error", "error": str(e)})
