...
async def get_strategies(
    ...
    try:
        ...
        logger.info(
            f"Получен список из {len(strategies)} стратегий",
            extra={"category": category, "active_only": active_only},
        )
...
async def configure_strategy(config: StrategyConfig):
    ...
    try:
        ...
        if not strategy_registry.get_strategy_class(config.strategy_name):
            raise HTTPException(
                status_code=404, detail=f"Стратегия {config.strategy_name} не найдена"
            )
        ...
        logger.info(
            ...
            extra={"strategy_config": strategy_config},
        )
...
async def start_backtest(request: BacktestRequest):
    ...
    try:
        ...
        if request.start_date >= request.end_date:
            raise HTTPException(
                status_code=400, detail="Дата начала должна быть меньше даты окончания"
            )

        if request.initial_balance <= 0:
            raise HTTPException(
                status_code=400, detail="Начальный баланс должен быть положительным"
            )
        ...
        if not strategy_registry.get_strategy_class(request.strategy_name):
            raise HTTPException(
                status_code=404, detail=f"Стратегия {request.strategy_name} не найдена"
            )
        ...
        logger.info(
            f"Запущен бэктест {backtest_id} для стратегии {request.strategy_name}",
            extra={"backtest_config": backtest_config},
        )
        ...
        categories = [
            {
                ...
                "description": "Стратегии на основе машинного обучения",
            },
            {
                "name": "indicator",
                "display_name": "Technical Indicators",
                "description": "Стратегии на технических индикаторах",
            },
            {
                "name": "arbitrage",
            ...
            },
            {
                ...
                "description": "Скальпинговые стратегии",
            },
            {
                "name": "grid",
                "display_name": "Grid Trading",
                "description": "Сеточные стратегии",
            },
            {
                "name": "trend",
                "display_name": "Trend Following",
                "description": "Трендовые стратегии",
            },
            {
                "name": "mean_reversion",
