...
async def check_account_mode(client: BybitClient):
    ...
    try:
        ...
        response = await client._make_request("GET", "/v5/account/info", auth=True)
        ...
        for symbol in symbols:
            ...
            if positions:
                for pos in positions:
                    ...
                    logger.info(
                        f"  Position Index: {position_idx} (0=OneWay, 1=BuyHedge, 2=SellHedge)"
                    )
            ...
            try:
                ...
                response = await client._make_request(
                    "GET", "/v5/account/position-mode", params, auth=True
                )
...
async def fix_order_parameters(client: BybitClient, symbol: str = "BTCUSDT"):
    ...
    try:
        ...
        for config in test_configs:
            ...
            try:
                response = await client._make_request(
                    "POST", "/v5/order/create", config["params"], auth=True
                )
                ...
                if response.get("retCode") == 0:
                    ...
                    if order_id:
                        ...
                        try:
                            ...
                            cancel_response = await client._make_request(
                                "POST", "/v5/order/cancel", cancel_params, auth=True
                            )
                    ...
                    logger.warning(
                        f"❌ Failed: {response.get('retMsg')} (code: {response.get('retCode')})"
                    )
...
async def main():
    ...
    client = get_bybit_client(api_key=api_key, api_secret=api_secret, sandbox=False)
