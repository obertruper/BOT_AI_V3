...
class RiskManager:
    ...
    async def _initial_risk_check(self):
        ...
        try:
            ...
            if risk_status.overall_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
                self.logger.warning(f"Обнаружен высокий уровень риска при инициализации: {risk_status.message}")
    ...
    async def check_signal_risk(self, signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not await self.position_risk.check_signal_risk(signal):
                self.logger.warning(f"Сигнал отклонен по позиционному риску: {signal.get('id')}")
                return False
            
            # Проверка портфельного риска
            if not await self.portfolio_exposure.check_signal_risk(signal):
                self.logger.warning(f"Сигнал отклонен по портфельному риску: {signal.get('id')}")
                return False
            ...
            if concentration_risk > self.config.get('max_concentration_risk', 0.3):
                self.logger.warning(f"Сигнал отклонен по концентрации риска: {concentration_risk:.2f}")
                return False
    ...
    async def check_global_risks(self) -> RiskStatus:
        ...
        try:
            ...
            if portfolio_risk:
                ...
                risk_scores.extend([alert.current_value or 0 for alert in portfolio_risk])
            
            # 2. Позиционные риски
            position_risks = await self._check_position_risks()
            if position_risks:
                alerts.extend(position_risks)
                risk_scores.extend([alert.current_value or 0 for alert in position_risks])
            
            # 3. Риски ликвидности
            liquidity_risks = await self._check_liquidity_risks()
            if liquidity_risks:
                alerts.extend(liquidity_risks)
                risk_scores.extend([alert.current_value or 0 for alert in liquidity_risks])
            ...
            if correlation_risks:
                ...
                risk_scores.extend([alert.current_value or 0 for alert in correlation_risks])
        ...
        try:
            ...
            if metrics.max_drawdown > max_drawdown_limit:
                alerts.append(RiskAlert(
                    ...
                    level=RiskLevel.HIGH if metrics.max_drawdown < max_drawdown_limit * 1.5 else RiskLevel.CRITICAL,
            ...
            if metrics.margin_usage > margin_limit:
                alerts.append(RiskAlert(
                    ...
                    level=RiskLevel.HIGH if metrics.margin_usage < margin_limit * 1.2 else RiskLevel.CRITICAL,
            ...
            if metrics.concentration_risk > concentration_limit:
                alerts.append(RiskAlert(
                    ...
                    level=RiskLevel.MEDIUM if metrics.concentration_risk < concentration_limit * 1.5 else RiskLevel.HIGH,
    ...
    async def _check_position_risks(self) -> List[RiskAlert]:
        ...
        try:
            ...
            for position in positions:
                ...
                portfolio_value = await self.position_manager.get_total_portfolio_value()
                ...
                if portfolio_value > 0:
                    ...
                    if position_percent > max_position_percent:
                        alerts.append(RiskAlert(
                            ...
                            level=RiskLevel.MEDIUM if position_percent < max_position_percent * 1.5 else RiskLevel.HIGH,
                ...
                if position.unrealized_pnl_percent < -self.config.get('max_position_loss_percent', 15.0):
                    alerts.append(RiskAlert(
            ...
            for exchange_name in self.exchange_registry.get_exchange_names():
                ...
                if exchange:
                    ...
                    if total_balance > 0:
                        ...
                        min_free_percent = self.config.get('min_free_balance_percent', 10.0)
        ...
        try:
            ...
            if correlation_risk > max_correlation:
                alerts.append(RiskAlert(
                    ...
                    level=RiskLevel.MEDIUM if correlation_risk < max_correlation * 1.2 else RiskLevel.HIGH,
    ...
    def _determine_risk_action(self, level: RiskLevel, alerts: List[RiskAlert]) -> tuple[bool, RiskAction]:
    ...
    async def _handle_critical_risk(self, risk_status: RiskStatus):
        ...
        try:
            ...
            if self._last_emergency_action:
                time_since_last = (datetime.now() - self._last_emergency_action).total_seconds()
                if time_since_last < self.emergency_cooldown:
                    self.logger.warning(f"Экстренное действие отложено (cooldown: {time_since_last:.0f}с)")
                    return
        ...
        try:
            ...
            for position in positions:
                if position.size != 0:
                    ...
                    await self.position_manager.reduce_position(position.symbol, abs(new_size))
    ...
    async def _process_new_alerts(self, alerts: List[RiskAlert]):
        ...
        for alert in alerts:
            ...
            self.logger.log(log_level, f"RISK ALERT [{alert.level.value.upper()}]: {alert.message}")
    ...
    async def get_risk_metrics(self) -> RiskMetrics:
        ...
        try:
            # Проверка кеша
            now = datetime.now()
            if (self._metrics_cache and self._last_metrics_update and
                (now - self._last_metrics_update).total_seconds() < self._metrics_cache_ttl):
            ...
            # Портфельные метрики
            metrics.portfolio_value = await self.position_manager.get_total_portfolio_value()
            metrics.total_exposure = await self.portfolio_exposure.calculate_total_exposure()
            ...
            # Метрики риска
            metrics.correlation_risk = await self.calculator.calculate_correlation_risk()
            metrics.concentration_risk = await self.calculator.calculate_concentration_risk()
            ...
            # Общий риск-скор
            metrics.risk_score = await self.calculator.calculate_overall_risk_score(metrics)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            if self.portfolio_exposure and not await self.portfolio_exposure.health_check():
                return False
