...
async def test_hedge_mode():
    ...
    for side, description in test_cases:
        ...
        print(f"{status} {description:15} | side={side:5} | position_idx={position_idx} | expected={expected}")
    ...
    if position_idx != 0:
        ...
        if os.getenv("BYBIT_HEDGE_MODE", "true").lower() == "true":
            ...
            print(f"✅ One-way mode корректно определён: positionIdx=0")
...
async def test_integration():
    ...
    for env_value in test_env_values:
        ...
        print(f"{status} BYBIT_HEDGE_MODE={env_value:5} | hedge_mode={actual_hedge} | expected={expected_hedge}")
