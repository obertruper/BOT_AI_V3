...
from .instrument_settings import (
...
class InstrumentManager:
    ...
    def get_instrument_info(self, symbol: str, force_refresh: bool = False) -> dict[str, Any]:
        ...
        with self._cache_lock:
            ...
            if self.api_client:
                try:
                    logger.info(
                        f"Fetching instrument info from API for {clean_symbol} (original: {symbol})"
                    )
    ...
    def _fetch_from_api(self, symbol: str) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if lot_size_filter:
                ...
                result["max_qty"] = float(
                    lot_size_filter.get("maxOrderQty", DEFAULT_SETTINGS["max_qty"])
                )
                result["max_market_qty"] = float(
                    lot_size_filter.get("maxMktOrderQty", DEFAULT_SETTINGS["max_market_qty"])
                )
            ...
            if notional_filter and "min_notional" not in result:
                result["min_notional"] = float(
                    notional_filter.get("minNotional", DEFAULT_SETTINGS["min_notional"])
                )
    ...
    def round_qty(
        self, symbol: str, qty: float, round_up: bool = False, enforce_min: bool = True
    ) -> float:
        """
        ...
        logger.info(
            f"round_qty для {symbol}: {qty} -> {rounded_qty} (шаг: {qty_step}, round_up: {round_up})"
        )
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        with self._cache_lock:
            ...
            stats = {"total_symbols": len(self._cache), "cache_ttl": self.cache_ttl, "symbols": {}}
