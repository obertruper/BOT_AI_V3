            ...
            if instrument_info:
                ...
                if price_filter:
                    try:
                        ...
                        logger.error(
                            f"[get_instrument_settings] => Ошибка при обработке priceFilter: {e}"
                        )
                ...
                if lot_size_filter:
                    try:
                        ...
                        if "maxMktOrderQty" in lot_size_filter:
                            api_settings["maxMktOrderQty"] = float(
                                lot_size_filter["maxMktOrderQty"]
                            )
                    except (ValueError, TypeError) as e:
                        logger.error(
                            f"[get_instrument_settings] => Ошибка при обработке lotSizeFilter: {e}"
                        )
                ...
                if notional_filter:
                    try:
                        ...
                        logger.error(
                            f"[get_instrument_settings] => Ошибка при обработке notionalFilter: {e}"
                        )
    ...
    if api_settings:
        ...
        if symbol == "ETHUSDT":
            ...
            if api_settings.get("minOrderQty", 0.001) < 0.01:
                logger.warning(
                    f"[get_instrument_settings] => Для ETHUSDT API вернул minOrderQty={api_settings.get('minOrderQty')}, корректируем до 0.01"
                )
            ...
            if api_settings.get("qtyStep", 0.001) < 0.01:
                logger.warning(
                    f"[get_instrument_settings] => Для ETHUSDT API вернул qtyStep={api_settings.get('qtyStep')}, корректируем до 0.01"
                )
        ...
        if "ALGO" in symbol and api_settings.get("qtyStep", 0.001) < 0.1:
            logger.warning(
                f"[get_instrument_settings] => Для {symbol} API вернул qtyStep={api_settings.get('qtyStep')}, корректируем до 0.1"
            )
        ...
        for key in ["minOrderQty", "qtyStep"]:
            ...
            if key in api_settings and key in table_settings:
                ...
                if api_value < table_value / 2:
                    logger.warning(
                        f"[get_instrument_settings] => Настройка {key} из API ({api_value}) для {symbol} подозрительно мала, используем значение из таблицы ({table_value})"
                    )
