...
class PartialTPManager:
    ...
    async def _place_partial_order(self, order_data: Dict) -> bool:
        ...
        try:
            ...
            from exchanges.base.order_types import OrderRequest, OrderSide as ExchangeOrderSide, OrderType as ExchangeOrderType
            
            order_request = OrderRequest(
                symbol=order_data["symbol"],
                side=ExchangeOrderSide.SELL if order_data["side"] == OrderSide.SELL else ExchangeOrderSide.BUY,
                    ...
                    if side == "long":
                        ...
                        if profit_ratio >= activation_profit:
                            ...
                            if trailing_stop_price is None or new_stop > trailing_stop_price:
                                ...
                                await self._update_stop_loss(symbol, side, float(trailing_stop_price))
    ...
    async def check_partial_execution(self, symbol: str, side: str) -> Optional[Dict]:
        """
