...
class DynamicSLTPCalculator:
    ...
    def calculate_dynamic_levels(
        ...
        try:
            ...
            if session_multiplier < 1.0:
                ...
                self.logger.debug(f"Азиатская сессия: уровни уменьшены на {(1-session_multiplier)*100:.0f}%")
    ...
    def _calculate_atr_volatility(
        ...
        try:
            ...
            self.logger.debug(
                f"ATR={atr:.4f}, Ratio={volatility_ratio:.4f}, "
    ...
    def _get_rsi_factor(self, indicators: Optional[Dict]) -> float:
    ...
    def _get_volume_factor(self, indicators: Optional[Dict]) -> float:
    ...
    def _calculate_partial_tp_levels(
        ...
        for i, (ratio, percent) in enumerate(zip(level_ratios, level_percents)):
