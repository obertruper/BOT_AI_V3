...
1. **BotAIV3Application** (`main.py:32`) - главная точка входа, управляет жизненным циклом
2. **SystemOrchestrator** (`core/system/orchestrator.py:43`) - системная координация всех компонентов
3. **TradingEngine** (`trading/engine.py:597`) - центральная координация всех торговых операций
4. **StrategyManager** (`strategies/manager.py:639`) - жизненный цикл стратегий с горячей заменой
5. **RiskManager** (`risk_management/manager.py:857`) - real-time мониторинг рисков с автоматическими действиями
6. **ExchangeRegistry** (`exchanges/registry.py`) - унифицированный доступ к 7 биржам
7. **Database connections** (`database/connections/postgres.py`) - синхронное и асинхронное подключение к БД
