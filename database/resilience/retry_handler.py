...
class RetryConfig:
    ...
    stop_exceptions: list[type[Exception]] = field(
        default_factory=lambda: [
    ...
    def update(self, attempts: list[RetryAttempt], final_success: bool):
        ...
        for attempt in attempts:
            if attempt.exception:
                ...
                self.most_common_exceptions[exc_name] = (
                    self.most_common_exceptions.get(exc_name, 0) + 1
                )
...
class DatabaseRetryHandler:
    ...
    def __init__(self, name: str, config: RetryConfig | None = None):
        ...
        logger.info(
            f"Retry handler '{name}' initialized with {self.config.max_attempts} max attempts"
        )
        ...
        for attempt_number in range(1, self.config.max_attempts + 1):
            ...
            try:
                ...
                if self._is_deadlock_error(e):
                    ...
                    logger.info(f"Deadlock detected, waiting {deadlock_delay:.2f}s before retry")
...
class DatabaseRetryManager:
    ...
    def get_handler(self, name: str) -> DatabaseRetryHandler | None:
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "overall_failure_rate": (
                (total_failures / total_operations * 100) if total_operations > 0 else 0
            ),
            "worst_performing_handler": (
                {"name": worst_handler, "failure_rate": worst_failure_rate}
                if worst_handler
                else None
            ),
        }
...
def with_retry(name: str, config: RetryConfig | None = None):
    """
...
FAST_OPERATION_CONFIG = RetryConfig(
    max_attempts=3, base_delay=0.1, max_delay=2.0, backoff_strategy=BackoffStrategy.EXPONENTIAL
)

BULK_OPERATION_CONFIG = RetryConfig(
    max_attempts=3, base_delay=2.0, max_delay=60.0, backoff_strategy=BackoffStrategy.LINEAR
)
