...
class QueryOptimizer:
    ...
    async def _cleanup_expired_entries(self):
        ...
        async with self._prep_lock:
            expired_statements = [
                key
                for key, stmt in self._prepared_statements.items()
    ...
    async def _get_or_create_prepared_statement(self, conn: asyncpg.Connection, query: str) -> str:
        """
    ...
    async def _update_metrics(
        self, query: str, execution_time_ms: float, cached: bool = False, error: bool = False
    ):
        """
    ...
    async def get_query_stats(self) -> dict[str, Any]:
        ...
        slowest_queries = sorted(metrics, key=lambda m: m.avg_time_ms, reverse=True)[:10]

        # Find most frequent queries
        most_frequent = sorted(metrics, key=lambda m: m.execution_count, reverse=True)[:10]
            ...
            "most_frequent_queries": [
                {
                    ...
                    "avg_time_ms": m.avg_time_ms,
                    "cache_hit_rate": (
                        (m.cache_hits / m.execution_count * 100) if m.execution_count > 0 else 0
                    ),
                }
    ...
    async def optimize_suggestions(self) -> list[dict[str, Any]]:
        ...
        for metric in metrics:
            ...
            if metric.execution_count > 100 and metric.avg_time_ms > self.slow_query_threshold_ms:
                suggestions.append(
                    {
            ...
            if (
                metric.execution_count > 50
                and metric.cache_hits == 0
                and "SELECT" in metric.query_template
            ):
                suggestions.append(
                    {
