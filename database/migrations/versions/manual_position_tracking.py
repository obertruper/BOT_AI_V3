"""Add position tracking tables manually

Revision ID: manual_position_tracking
Revises: 8fc5ad5d2292
Create Date: 2025-08-20 11:50:00.000000
...
revision = 'manual_position_tracking'
...
def upgrade() -> None:
    ...
    # Проверяем, существует ли таблица tracked_positions
    connection = op.get_bind()
    inspector = sa.inspect(connection)
    existing_tables = inspector.get_table_names()
    
    # Таблица для отслеживания позиций
    if 'tracked_positions' not in existing_tables:
    ...
    if 'position_metrics_history' not in existing_tables:
        op.create_table('position_metrics_history',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('position_id', sa.String(50), nullable=False),
        ...
        op.create_index('ix_position_metrics_history_position_id', 'position_metrics_history', ['position_id'])
        op.create_index('ix_position_metrics_history_timestamp', 'position_metrics_history', ['timestamp'])

    # Таблица для системных алертов
    if 'position_alerts' not in existing_tables:
        op.create_table('position_alerts',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('position_id', sa.String(50), nullable=False),
