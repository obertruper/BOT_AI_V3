...
class TransactionManager:
    ...
    async def transaction(self, isolation_level: str = "read_committed"):
        ...
        metrics = TransactionMetrics(transaction_id=transaction_id, start_time=time.time())
        ...
        try:
            ...
            # Set isolation level
            await conn.execute(
                f"SET TRANSACTION ISOLATION LEVEL {isolation_level.upper().replace('_', ' ')}"
            )
        ...
        while retry_count < max_retries:
            try:
                ...
                if retry_count < max_retries:
                    ...
                    logger.error(
                        f"Max retries exceeded for transaction after {max_retries} attempts"
                    )
                    raise
    ...
    async def get_transaction_stats(self) -> dict[str, Any]:
        ...
        return {
            "active_transactions": len(
                [t for t in active_txns if t.state == TransactionState.ACTIVE]
            ),
            ...
            "committed_transactions": len(
                [t for t in active_txns if t.state == TransactionState.COMMITTED]
            ),
        }
    ...
    async def commit(self) -> list[Any]:
        ...
        try:
            results = await self.transaction_manager.execute_in_transaction(
                self._operations, isolation_level="read_committed"
            )
...
async def create_order_with_trade_atomic(
    transaction_manager: TransactionManager, order_data: dict, trade_data: dict
) -> tuple[int, int]:
    """
    ...
    results = await transaction_manager.execute_in_transaction([create_order, create_trade])
