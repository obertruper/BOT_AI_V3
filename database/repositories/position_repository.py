...
class PositionRepository(BaseRepository[Position]):
    ...
    def _from_record(self, record: asyncpg.Record) -> Position:
        ...
        return Position(
            ...
            current_price=(
                Decimal(str(record["current_price"])) if record["current_price"] else None
            ),
            ...
            pnl_percentage=(
                Decimal(str(record["pnl_percentage"])) if record["pnl_percentage"] else None
            ),
    ...
    async def open_position(self, position: Position, order_id: int | None = None) -> int:
        """
        ...
        async with self.transaction_manager.transaction() as conn:
            ...
            logger.info(
                f"Opened position {position_id} for {position.symbol} on {position.exchange}"
            )
            return position_id

    async def update_position_price(
        self, position_id: int, current_price: Decimal, calculate_pnl: bool = True
    ) -> dict[str, Any]:
        """
    ...
    async def close_position(
        ...
        async with self.transaction_manager.transaction() as conn:
            ...
            await conn.execute(
                "UPDATE orders SET status = 'closed' WHERE position_id = $1", position_id
            )
    ...
    async def update_positions_batch(self, updates: list[tuple[int, dict[str, Any]]]) -> int:
        """
        ...
        formatted_updates = [
            ({"id": position_id}, update_data) for position_id, update_data in updates
        ]
    ...
    async def get_position_stats(
        ...
        async with (
            ...
            return {
                ...
                "avg_pnl_percentage": (
                    float(record["avg_pnl_percentage"]) if record["avg_pnl_percentage"] else 0
                ),
