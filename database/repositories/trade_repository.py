...
class TradeRepository(BaseRepository[Trade]):
    ...
    async def get_recent_trades(
        ...
        query_parts.extend(["ORDER BY executed_at DESC", f"LIMIT {limit}"])
    ...
    async def get_daily_pnl(
        self, days: int = 30, exchange: str | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    async def get_symbol_performance(self, limit: int = 20, days: int = 7) -> list[dict[str, Any]]:
        """
            ...
            return [
                {
                    ...
                    "win_rate": (
                        (record["winning_trades"] / record["trades_count"] * 100)
                        if record["trades_count"] > 0
                        else 0
                    ),
                }
                for record in records
