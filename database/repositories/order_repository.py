...
class OrderRepository(BaseRepository[Order]):
    ...
    async def create_order(self, order: Order, signal_id: int | None = None) -> int:
        """
        ...
        if self.transaction_manager:
            async with self.transaction_manager.transaction() as conn:
                ...
                if signal_id:
                    await conn.execute(
                        "UPDATE signals SET order_id = $1 WHERE id = $2", order_id, signal_id
                    )
        else:
            async with (
    ...
    async def cancel_orders(self, order_ids: list[int], reason: str = "manual_cancel") -> int:
        """
        ...
        async with (
            ...
            result = await conn.execute(query, datetime.now(), metadata_update, order_ids)
    ...
    async def bulk_update_orders(self, updates: list[tuple[int, dict[str, Any]]]) -> int:
        """
        ...
        formatted_updates = [({"id": order_id}, update_data) for order_id, update_data in updates]
        ...
        async with (
            ...
            return {
                ...
                "avg_execution_time_seconds": (
                    float(record["avg_execution_time_seconds"])
                    if record["avg_execution_time_seconds"]
                    else 0
                ),
    ...
    async def cleanup_old_orders(self, days_to_keep: int = 30, keep_filled: bool = True) -> int:
        """
