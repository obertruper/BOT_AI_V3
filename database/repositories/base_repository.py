...
class BaseRepository(Generic[T], ABC):
    ...
    def __init__(
        self, pool: asyncpg.Pool, table_name: str, model_class: type[T], transaction_manager=None
    ):
        """
    ...
    async def bulk_insert(
        ...
        for i in range(0, total_items, chunk_size):
            ...
            chunk_results = await self._insert_chunk(chunk, returning_fields, on_conflict)
    ...
    async def _insert_chunk(
        self, chunk: list[T], returning_fields: list[str] | None, on_conflict: str | None
    ) -> list[dict[str, Any]]:
        ...
        values_template = "({})".format(",".join([f"${i}" for i in range(1, len(columns) + 1)]))
    ...
    async def bulk_update(
        self, updates: list[tuple[dict[str, Any], dict[str, Any]]], chunk_size: int = 500
    ) -> int:
        """
    ...
    async def _update_chunk(self, chunk: list[tuple[dict[str, Any], dict[str, Any]]]) -> int:
    ...
    async def bulk_delete(self, conditions: list[dict[str, Any]], chunk_size: int = 1000) -> int:
        """
    ...
    async def bulk_upsert(
        ...
        return await self.bulk_insert(
            items, returning_fields=["id"], on_conflict=on_conflict, chunk_size=chunk_size
        )

    async def execute_in_transaction(
        self, operations: list[Callable], conn: asyncpg.Connection | None = None
    ) -> list[Any]:
        """
    ...
    async def get_batch_by_ids(self, ids: list[int | str], id_column: str = "id") -> list[T]:
        """
