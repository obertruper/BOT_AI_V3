...
async def example_basic_operations():
    ...
    recent_predictions = await db.ml_predictions.get_recent_predictions(symbol="BTC/USDT", hours=1)
...
async def example_transaction_operations():
    ...
    async def log_prediction_for_position(conn):
        prediction = MLPrediction(symbol="ETH/USDT", prediction="BUY", confidence=0.9)
...
async def example_monitoring_and_metrics():
    ...
    if "circuit_breakers" in stats:
        ...
        print(
            f"Circuit breakers: {cb_health['total_breakers']} total, {cb_health['open_breakers']} open"
        )
...
async def example_position_management():
    ...
    update_result = await db.positions.update_position_price(position_id, Decimal("46000"))
...
async def example_ml_prediction_analytics():
    ...
    for symbol_data in top_symbols:
        print(
            f"  {symbol_data['symbol']}: {symbol_data['avg_expected_return']:.2f}% expected return"
        )
