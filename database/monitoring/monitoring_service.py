...
class DatabaseMonitoringService:
    ...
    def _store_metrics(self, category: str, metrics: dict[str, float], timestamp: datetime):
        ...
        for metric_name, value in metrics.items():
            ...
            self._metrics_history[key].append({"timestamp": timestamp, "value": value})
    ...
    async def _evaluate_alert_rules(self):
        ...
        for rule_name, rule in self._alert_rules.items():
            ...
            if (
                rule.last_triggered
                and (current_time - rule.last_triggered).total_seconds() < rule.cooldown_seconds
            ):
                continue
    ...
    def _get_current_metric_value(self, metric_path: str) -> float | None:
    ...
    async def get_metrics_history(
        self, metric_path: str, minutes: int = 60
    ) -> list[dict[str, Any]]:
        """
    ...
    async def _generate_recommendations(self) -> list[str]:
        ...
        if long_txns and long_txns > 0:
            recommendations.append(
                "Long-running transactions detected, review transaction boundaries"
            )
