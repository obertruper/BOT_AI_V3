...
class DatabaseManager:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.ml_prediction_repository: MLPredictionRepository | None = None
        self.position_repository: PositionRepository | None = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.ml_prediction_repository = MLPredictionRepository(
                self.pool, self.transaction_manager
            )
    ...
    async def create_order_with_trade_atomic(
        self, order_data: dict[str, Any], trade_data: dict[str, Any]
    ) -> tuple[int, int]:
        """
        ...
        results = await self.transaction_manager.execute_in_transaction(
            [create_order_operation, create_trade_operation]
        )
...
async def get_database_manager(config: dict[str, Any] | None = None) -> DatabaseManager:
    """
...
async def example_atomic_operations():
    ...
    async with DatabaseContext() as db_manager:
        ...
        order_data = {"symbol": "BTC/USDT", "side": "buy", "quantity": 1.0, "price": 50000.0}

        trade_data = {"symbol": "BTC/USDT", "side": "buy", "quantity": 1.0, "price": 50000.0}

        order_id, trade_id = await db_manager.create_order_with_trade_atomic(order_data, trade_data)
