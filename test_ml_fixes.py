...
async def test_ml_predictions():
    ...
    try:
        ...
        logger.info(f"     15m: {prediction.get('direction_15m', 'N/A')} (conf: {prediction.get('confidence_15m', 0):.2%})")
        logger.info(f"     1h:  {prediction.get('direction_1h', 'N/A')} (conf: {prediction.get('confidence_1h', 0):.2%})")
        logger.info(f"     4h:  {prediction.get('direction_4h', 'N/A')} (conf: {prediction.get('confidence_4h', 0):.2%})")
        logger.info(f"     12h: {prediction.get('direction_12h', 'N/A')} (conf: {prediction.get('confidence_12h', 0):.2%})")
    ...
    test_features_short[:, 1] = -np.abs(np.sin(np.linspace(0, 2*np.pi, 96)) * 0.1)  # Отрицательная волатильность
