...
def test_signal_quality():
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        print(f"   Размер позиции: {result['recommendation']['position_size_multiplier'] * 100:.0f}%")
        ...
        print(f"   15m: {details['15m']}, 1h: {details['1h']}, 4h: {details['4h']}, 12h: {details['12h']}")
        
        if 'LONG_count' in details:
            print(f"   LONG: {details.get('LONG_count', 0)}, "
    ...
    print(f"Среднее качество: {stats['medium_quality']} ({stats.get('medium_quality_pct', 0):.1f}%)")
