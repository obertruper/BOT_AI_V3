...
async def test_order_creation(client: BybitClient):
    ...
    try:
        ...
        params = {"accountType": "UNIFIED"}  # или CONTRACT для контрактного счета
        ...
        logger.info(f"\n{'='*60}")
        logger.info("Creating test MARKET BUY order:")
        ...
        if response.get("retCode") == 0:
            ...
            if order_id:
                ...
                status_params = {"category": "linear", "orderId": order_id}
                status_response = await client._make_request(
                    "GET", "/v5/order/realtime", status_params, auth=True
                )
