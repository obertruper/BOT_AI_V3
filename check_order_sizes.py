...
async def check_order_sizes():
    ...
    try:
        ...
        print("-" * 80)
        print(f"{'Символ':<15} {'Цена':<12} {'Мин. кол-во':<15} {'Мин. в USDT':<12} {'Шаг цены':<12}")
        print("-" * 80)
        ...
        for symbol in symbols:
            if symbol in markets:
                ...
                # Шаг цены
                price_precision = market['precision']['price'] if market['precision']['price'] else 0.01
                # Шаг количества
                amount_precision = market['precision']['amount'] if market['precision']['amount'] else 0.001
                ...
                print(f"{display_symbol:<15} ${price:<11.2f} {min_amount:<15.6f} ${min_value_usdt:<11.2f} {price_precision:<12}")
                
                # Проверяем, можем ли открыть позицию с текущим балансом
                if min_value_usdt > available_usdt:
                    print(f"   ⚠️ Недостаточно средств для открытия минимальной позиции")
                
                # Рекомендуемый размер (с учетом комиссий и запаса)
                recommended_size = max(min_value_usdt * 1.2, 6)  # Минимум 6 USDT или 120% от минимума
                if recommended_size <= available_usdt:
                    recommended_qty = recommended_size / price
                    # Округляем до правильной точности
                    if amount_precision > 0:
                        recommended_qty = round(recommended_qty / amount_precision) * amount_precision
                    print(f"   ✅ Рекомендуемый размер: {recommended_qty:.6f} ({recommended_size:.2f} USDT)")
