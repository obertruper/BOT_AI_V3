...
class TestRepository(BaseRepository[TestModel]):
    ...
    def _to_dict(self, model: TestModel) -> dict[str, Any]:
        ...
        result = {"name": model.name, "value": model.value}
    ...
    def _from_record(self, record: asyncpg.Record) -> TestModel:
        ...
        return TestModel(
            id=record.get("id"), name=record.get("name", "test"), value=record.get("value", 0.0)
        )
...
class TestBaseRepository:
    ...
    async def test_bulk_insert_success(self, test_repository, mock_transaction_manager):
        ...
        mock_conn = AsyncMock()
        mock_conn.fetch.return_value = [MagicMock(id=1), MagicMock(id=2)]
        ...
        # Test data
        items = [TestModel(name="test1", value=1.0), TestModel(name="test2", value=2.0)]

        results = await test_repository.bulk_insert(items=items, returning_fields=["id"])
    ...
    async def test_bulk_insert_with_conflict(self, test_repository, mock_transaction_manager):
        ...
        results = await test_repository.bulk_insert(
            items=items, on_conflict="ON CONFLICT DO NOTHING"
        )
    ...
    async def test_bulk_upsert_success(self, test_repository):
        ...
        with patch.object(test_repository, "bulk_insert") as mock_bulk_insert:
            ...
            results = await test_repository.bulk_upsert(
                items=items, conflict_columns=["name"], update_columns=["value"]
            )
    ...
    async def test_get_batch_by_ids_success(self, test_repository, mock_transaction_manager):
        ...
        mock_records = [
            MagicMock(
                get=MagicMock(
                    side_effect=lambda k, d=None: {"id": 1, "name": "test1", "value": 1.0}.get(k, d)
                )
            ),
            MagicMock(
                get=MagicMock(
                    side_effect=lambda k, d=None: {"id": 2, "name": "test2", "value": 2.0}.get(k, d)
                )
            ),
        ]
...
class TestCreateOrderWithTradeAtomic:
    ...
    async def test_create_order_with_trade_atomic_success(self, mock_pool):
        ...
        order_id, trade_id = await create_order_with_trade_atomic(manager, order_data, trade_data)
