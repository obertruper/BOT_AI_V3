...
from database.models.base_models import SignalType, OrderSide, OrderStatus
...
class TestDynamicSLTPEndToEnd:
    ...
    async def test_full_trading_pipeline(self, full_system_setup, realistic_market_data):
        ...
        print(f"   Dynamic SL: ${trading_signal['stop_loss']:.2f} ({dynamic_levels['stop_loss_pct']:.2f}%)")
        print(f"   Dynamic TP: ${trading_signal['take_profit']:.2f} ({dynamic_levels['take_profit_pct']:.2f}%)")
    ...
    async def test_volatility_adaptation_scenario(self, full_system_setup):
        ...
        print(f"   Low Vol  - SL: {low_vol_levels['stop_loss_pct']:.2f}%, TP: {low_vol_levels['take_profit_pct']:.2f}%")
        print(f"   High Vol - SL: {high_vol_levels['stop_loss_pct']:.2f}%, TP: {high_vol_levels['take_profit_pct']:.2f}%")
        print(f"   Adaptation: SL +{high_vol_levels['stop_loss_pct'] - low_vol_levels['stop_loss_pct']:.2f}%, TP +{high_vol_levels['take_profit_pct'] - low_vol_levels['take_profit_pct']:.2f}%")
    ...
    async def test_error_handling_and_fallback(self, full_system_setup):
        ...
        print(f"   Insufficient data fallback: {result_insufficient['volatility_data']['volatility_regime']}")
        print(f"   Corrupted data recovery: SL={result_corrupted['stop_loss_pct']:.2f}%, TP={result_corrupted['take_profit_pct']:.2f}%")
    
    def _create_market_data_with_volatility(self, volatility_level: str, periods: int) -> pd.DataFrame:
