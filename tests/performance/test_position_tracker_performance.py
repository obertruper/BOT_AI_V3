...
class TestPositionTrackerPerformance:
    ...
    async def test_position_removal_performance(self, performance_tracker):
        ...
        # Удаляем все позиции
        removal_tasks = [
            performance_tracker.remove_position(pos_id, "closed") for pos_id in position_ids
        ]
...
class TestPositionTrackerStressTests:
    ...
    async def test_high_frequency_updates(self, stress_tracker):
        ...
        update_tasks = [
            stress_tracker.update_position_metrics("stress_hf_001") for _ in range(num_updates)
        ]
