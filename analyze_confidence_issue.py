...
async def analyze():
    ...
    if confidence_values:
        ...
        if pct_near_threshold > 80:
            ...
            print("\nВОЗМОЖНЫЕ ПРИЧИНЫ:")
            print(
                "1. Модель переобучена выдавать минимальное значение для прохождения порога"
            )
            print(
                "2. В коде есть явная корректировка confidence до порогового значения"
            )
            print(
                "3. Формула расчета confidence дает одинаковый результат для разных входов"
            )
            print("\nРЕКОМЕНДУЕМЫЕ ДЕЙСТВИЯ:")
            print(
                "1. Проверить расчет confidence_scores в модели (строки 633-640 в ml_manager.py)"
            )
